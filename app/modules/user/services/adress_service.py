...
class DeliveryAdressService:
    ...
    def _get_active_or_404(self, uow: UserUnitOfWork, id: int) -> DeliveryAdress:
    ...
    def _verify_ownership(self, address: DeliveryAdress, user_id: int) -> None:
    ...
    def create(self, user_id: int, data: DeliveryAdressCreate) -> DeliveryAdressRead:
        with UserUnitOfWork(self._session) as uow:
            ...
            result = DeliveryAdressRead.model_validate(address)
        return result

    def update(
        self, id: int, data: DeliveryAdressUpdate, user_id: int
    ) -> DeliveryAdressRead:
        with UserUnitOfWork(self._session) as uow:
    ...
    def restore(self, id: int, user_id: int) -> DeliveryAdressRead:
        with UserUnitOfWork(self._session) as uow:
            address = uow.delivery_adress.get_by_id(id)
    ...
    def get_active_by_user_id(self, user_id: int) -> list[DeliveryAdressRead]:
        with UserUnitOfWork(self._session) as uow:
            addresses = uow.delivery_adress.get_by_user_id(user_id, only_actives=True)
            return [DeliveryAdressRead.model_validate(a) for a in addresses]

    def get_by_user_id(self, user_id: int) -> list[DeliveryAdressRead]:
        with UserUnitOfWork(self._session) as uow:
